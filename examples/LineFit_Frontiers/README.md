...
- **`Figure2.ipynb`**  
  Runs LineFit on an example synthetic spectrum and produces **Figure 2** (fitted segments + recovered centres vs truth), illustrating multi-line extraction in a crowded window.

- **`Figure3.ipynb`**  
  Produces **Figure 3**: single-time-step method comparison of **LineFit vs four fast baseline estimators**:
...
- **`Figure4.ipynb`**  
  Produces **Figure 4**: time-series comparison (LOS-velocity fidelity), highlighting how occasional mis-tracking events can introduce excursions and bias time-domain diagnostics.

- **`Figure5.ipynb`**  
  Produces **Figure 5**: propagation into **wave diagnostics** by computing Refined Global Wavelet Spectra (RGWS) from recovered velocity time series and comparing against truth.
...
- **`FigureS1.ipynb`**  
  Generates the residual time series versus truth for the three representative lines shown in the main paper (Figure 4).
...
- **`Figures/`**  
  ...
  - `Figure1_LineFit_workflow.pdf`
  - `Figure2_synthetic_spectrum_fitted_cores.pdf`
  - `Figure3_methods_comparison.pdf`
  - `Figure4_velocities_comparison.pdf`
  - `Figure5_power_spectra_comparison.pdf`
  - `FigureS1_residual_timeseries_vs_truth.pdf`
  - `FigureS2_val_error_hist_cdf.pdf`
  - `FigureS3_rgws_line6.pdf`

- **`Files/`**  
  Cached outputs produced by the notebooks (used to avoid rerunning expensive steps):
  - `Fig4_cache/` — cached velocity time series used by Figure 4
  - `Fig5_cache/` — cached RGWS products used by Figure 5
  - `PoP_CNN_emulator_outputs/` — PoP emulator outputs (model pack, metrics, timing, diagnostics, plots)
  - `extracted_parameters_date_id_pixel_0.csv`, `fitted_spectra_date_id_pixel_0.fits` — example LineFit exports for Figure 2
